...
export function buildEffectiveBoms(records:ProgramRecord[],approved:BomDefinition[]){
  const pendingRecords=records.filter(record=>!record.completed);
  const approvedCodes=new Set(approved.map(bom=>bom.productCode));
  const provisional=[...new Set(pendingRecords.map(record=>record.productCode).filter(Boolean))]
    .filter(code=>!approvedCodes.has(code))
    .map(productCode=>({productCode,items:suggestBomFromProgram(pendingRecords,productCode).items}))
    .filter(bom=>bom.items.length>0);
  return{boms:[...approved,...provisional],approvedProducts:approved.length,provisionalProducts:provisional.length};
}
...
export function calculateRequirements(records: ProgramRecord[], boms: BomDefinition[]) {
  ...
  const completedOperations=records.filter(record=>record.completed).length;
  const pendingRecords=records.filter(record=>!record.completed);
  const mapped = pendingRecords.filter((record) => record.productCode && bomByProduct.has(record.productCode));
  const blocked = pendingRecords.filter((record) => !record.productCode || !bomByProduct.has(record.productCode));
