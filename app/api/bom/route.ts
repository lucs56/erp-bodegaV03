...
export async function GET() {
  ...
  } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "No se pudieron leer las fichas técnicas." }, { status: 500 }); }
}
export async function POST(request: Request) {
  try {
    ...
    if (!items.length || items.some((item) => !item.materialCode || !item.materialName || !item.action || !Number.isFinite(item.quantity) || item.quantity <= 0)) return Response.json({ error: "Cada ficha técnica necesita al menos un insumo válido con cantidad mayor que cero." }, { status: 400 });
  ...
  } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "No se pudo guardar la ficha técnica." }, { status: 500 }); }
}
